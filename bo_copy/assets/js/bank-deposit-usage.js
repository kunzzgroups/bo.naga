(function(){
  ...
  function render(methods,deposits,withdrawals,manualMovements){
    ...
    $('usageBankCount').textContent=methods.length.toLocaleString();$('usageApprovedCount').textContent=(deposits.length+manualDeposits.length).toLocaleString();$('usageApprovedAmount').textContent=money(deposits.reduce((a,r)=>a+num(r.amount),0)+manualDeposits.reduce((a,r)=>a+Math.abs(num(r.amount)),0));$('usageWithdrawAmount').textContent=money(withdrawals.reduce((a,r)=>a+num(r.amount),0)+manualWithdrawals.reduce((a,r)=>a+Math.abs(num(r.amount)),0));$('usageUnmatchedAmount').textContent=money(unmatchedDeposit+unmatchedWithdraw);
    const body=$('usageBody');body.innerHTML=filtered.length?filtered.map(m=>{const st=stats.get(String(m.id))||{deposit:0,depositCount:0,withdraw:0,withdrawCount:0};const net=st.deposit-st.withdraw,min=num(m.minAmount),max=num(m.maxAmount),daily=num(m.dailyLimit),pct=max>0?(Math.max(0,net)/max*100):0,fillClass=pct>=100?'over':pct>=80?'warn':'',cap=max>0?money(max):'No max',dailyText=daily>0?money(daily):'-';return `<tr><td><span class="usage-bank-name">${esc(m.bankName||m.displayName||'-')}</span><span class="usage-bank-sub">${esc(m.displayName||'-')} · ${esc(m.methodType||'-')}</span></td><td class="usage-account">${m.accountName?esc(m.accountName)+'<br>':''}${m.accountNumber?esc(m.accountNumber):'-'}</td><td><b>${money(min)}</b> - <b>${max>0?money(max):'No max'}</b></td><td><b>+${money(st.deposit)}</b><br><small>${st.depositCount.toLocaleString()} deposit movement(s)</small></td><td><b>-${money(st.withdraw)}</b><br><small>${st.withdrawCount.toLocaleString()} withdrawal movement(s)</small></td><td><div class="usage-meter"><div class="usage-meter-line"><span>${money(net)}</span><span>/ ${cap}</span></div>${max>0?`<div class="usage-track"><div class="usage-fill ${fillClass}" style="width:${Math.min(100,pct).toFixed(2)}%"></div></div>`:''}</div></td><td>${dailyText}</td><td>${max>0?`<b>${pct.toFixed(1)}%</b>`:'<span class="usage-muted">No max configured</span>'}</td><td><span class="usage-status ${Number(m.status)===1?'active':'inactive'}">${Number(m.status)===1?'ACTIVE':'INACTIVE'}</span></td></tr>`;}).join(''):'<tr><td colspan="9">No payment method found.</td></tr>';
    $('usageInfo').textContent=`${filtered.length} bank/method record(s) · ${unmatchedCount} unmatched approved transaction(s)`;
  }
  async function load(){const body=$('usageBody');if(body)body.innerHTML='<tr><td colspan="9">Loading...</td></tr>';try{const [methods,deposits,withdrawals,manualMovements]=await Promise.all([loadAllMethods(),loadAll('MEMBER_DEPOSIT_LIST'),loadAll('MEMBER_WITHDRAW_LIST'),loadManualBankMovements()]);render(methods,deposits,withdrawals,manualMovements);}catch(e){if(body)body.innerHTML='<tr><td colspan="9" class="text-danger">'+esc(e.message)+'</td></tr>';}}
