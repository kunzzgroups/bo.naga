...
function nowStamp(){const d=new Date();return String(d.getDate()).padStart(2,'0')+'/'+String(d.getMonth()+1).padStart(2,'0')+'/'+d.getFullYear()+' '+String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0')+':'+String(d.getSeconds()).padStart(2,'0');}
...
async function save(e){
 e.preventDefault();
 const body={id:$('policyId').value||null,policyKey:$('policyKey').value.trim(),tabLabel:$('tabLabel').value.trim(),title:$('policyTitle').value.trim(),lastUpdated:nowStamp(),sortOrder:Number($('sortOrder').value||0),status:Number($('policyStatus').value),contentHtml:$('contentHtml').value};
