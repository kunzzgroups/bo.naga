...
- **Not yet migrated: `main-merchant-balance.html`, `main-merchant-transactions.html`, `main_merchant_report.html`.** They load `main-merchant-report-executive.css`, not this family's stylesheet, so their picker still renders the retired palette — measured `#21A6D7` cells on a `#102030` panel, in both themes. The band JS works there (`is-start`/`is-end`/`is-preview` all applied); only the amber styling is missing, and that file is shared with the provider report pages, so repainting it is its own decision.
