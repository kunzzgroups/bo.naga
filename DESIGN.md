...
- **The five controls are one row, by policy** (a folded control row was read as a broken layout). Measured at 1932 / 1546 / 1440 / 1366 / 1280 / 1150: the controls never wrap among themselves; the **search box is the shock absorber** (a text field with a placeholder, it ellipsises cleanly) while the date control keeps its `260px` so its fixed-format label is never clipped at ≥1440; the **pills are never compressed**, so when the row genuinely runs out the two *groups* stack — pills on one line, all five controls together on the next — which is also what `main-admin-detail-executive.css` already did below 1279px. Below 1419px the date control may shrink too, with a `236px` floor.
