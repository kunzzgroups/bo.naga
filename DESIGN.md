...
**Payment Method Config** (`payment-method.html`): same Transaction listing table/pager · Edit/Delete `.bo-tx-action-btn` wells · amber QR View link. **Create / Edit** is a **full page** (`payment-method-create.html`) — not a modal · full main-column width · **Create Admin layer ladder** (card `#FFFCF7` · input `#FFF8EB` · sticky footer `#FFFCF7`) · viewport-fixed footer (Cancel Ghost + Create/Save Primary). **Bank Deposit Usage** twin: filter card + locked usage table · no title badges · Status = compact `.usage-status-chip` (**Active** / **Suspend** via `status`) · Show Mini Switch = bank display via **`visible`** (optimistic flip + session fallback) — **independent** controls. Full notes: system.md → Payment Method Config / Bank Deposit Usage / status chip / Show switch.
