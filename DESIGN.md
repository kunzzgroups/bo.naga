...
- **The five controls and the status pills share one line, by policy** (a folded control row was read as a broken layout, and so was a pills-only line). Measured at 1932 / 1700 / 1600 / 1546 / 1500 / 1440: one line down to **1546**, with 28px of slack left between the pills and the controls; at 1500 and below there is genuinely no room, so the two *groups* stack — pills on one line, all five controls together on the next — which is also what `main-admin-detail-executive.css` already did below 1279px. The controls never wrap among themselves at any width.
  Where the width comes from: the **search box is the shock absorber** (basis `132px`, capped `180px` — it ellipsises its own placeholder), the **date control keeps `248px`** so its fixed-format label never clips, and the two filter selects sit at their natural `160px`. **Those selects cannot be narrowed from CSS at all**: `reports.js` writes `width / min-width / max-width / flex:0 0 160px !important` **inline** on the `.rounded-select-wrap` it builds, and an inline important outranks any stylesheet important. Any further tightening has to come from the search, the date control, or `reports.js` itself.
