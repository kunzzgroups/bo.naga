...
Still a different component: **`.mas-avatar`** on the Security & Audit pages is a 50%-radius circle with an amber token wash and an `.is-alt` variant that alternates the tint by row position. It is already on the amber palette (it colours through tokens), but `.is-alt` is the same "colour encodes position" pattern that was removed here. Decide separately whether to unify it.
