(function(){
  ...
  $('effectList').addEventListener('click',async e=>{const editBtn=e.target.closest('[data-edit]'),delBtn=e.target.closest('[data-delete]');if(editBtn)return edit(editBtn.dataset.edit);if(delBtn){if(!confirm('Delete this animation rule?'))return;try{await json(base()+'/admin/animation-setting/delete?id='+encodeURIComponent(delBtn.dataset.delete),{method:'POST'});await loadRules();setStatus('Animation rule deleted. Refresh Naga and the original built-in behavior is restored after the latest rules are revalidated.')}catch(err){setStatus(err.message,true)}}});
