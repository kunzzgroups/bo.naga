(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{BO_AUTH.requireLogin();BO_AUTH.renderProfile&&BO_AUTH.renderProfile();BO_AUTH.renderSidebar&&BO_AUTH.renderSidebar();setToday();$('usageRefresh')?.addEventListener('click',load);$('usageFrom')?.addEventListener('change',load);$('usageTo')?.addEventListener('change',load);$('usageKeyword')?.addEventListener('keydown',e=>{if(e.key==='Enter')load();});$('usageKeyword')?.addEventListener('input',()=>{clearTimeout(window.__usageKwTimer);window.__usageKwTimer=setTimeout(load,280);});setTimeout(load,0);});
})();
