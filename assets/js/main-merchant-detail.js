(function(){'use strict';
 ...
 function providerRows(d){const out=$('madProviderPricingBody');if(!out)return;const assigned=new Map((d.providers||[]).map(x=>[String(x.providerCode||'').toUpperCase(),x]));const markup=Number(d.brand?.providerMarkupPercent||0);const platform=d.platformProviders||[];out.innerHTML=platform.length?platform.map(p=>{const code=String(p.code||'').toUpperCase(),bp=assigned.get(code),checked=!!bp&&String(bp.ownership||'PLATFORM').toUpperCase()==='PLATFORM',base=Number(p.settlementCostPercent||0),override=Number(bp?.defaultChargePercent||0),eff=(override>0?override:base)+markup,basis=String(bp?.chargeBasis||p.settlementCostBasis||'HOUSE_WIN').toUpperCase();return `<tr data-provider-row="${esc(code)}" data-base="${base}"><td><input type="checkbox" class="form-check-input" data-provider-use ${checked?'checked':''}></td><td><b>${esc(code)}</b><small class="d-block text-muted">${esc(p.name||'')}</small></td><td>${base.toFixed(4)}%</td><td><input type="number" class="form-control form-control-sm" min="0" max="100" step="0.0001" data-provider-override value="${override>0?override:''}" ${checked?'':'disabled'}></td><td data-provider-markup>${markup.toFixed(4)}%</td><td><b data-provider-effective>${eff.toFixed(4)}%</b></td><td>${basis==='TURNOVER'?'Turnover':'House Win / GGR'}</td></tr>`}).join(''):'<tr><td colspan="7" class="text-center text-muted py-3">No platform providers configured.</td></tr>';bindProviderInputs()}
 function bindProviderInputs(){const markup=$('madProviderMarkup');const recalc=()=>{const m=Number(markup?.value||0);document.querySelectorAll('[data-provider-row]').forEach(r=>{const use=r.querySelector('[data-provider-use]'),inp=r.querySelector('[data-provider-override]'),base=Number(r.dataset.base||0),ov=Number(inp?.value||0);if(inp)inp.disabled=!use?.checked;const mcell=r.querySelector('[data-provider-markup]'),eff=r.querySelector('[data-provider-effective]');if(mcell)mcell.textContent=m.toFixed(4)+'%';if(eff)eff.textContent=((ov>0?ov:base)+m).toFixed(4)+'%';});};markup?.addEventListener('input',recalc);document.querySelectorAll('[data-provider-use]').forEach(x=>x.addEventListener('change',recalc));document.querySelectorAll('[data-provider-override]').forEach(x=>x.addEventListener('input',recalc));recalc()}
 ...
 async function saveProviderPricing(id){const selected=[...document.querySelectorAll('[data-provider-row]')].filter(r=>r.querySelector('[data-provider-use]')?.checked);await api('/admin/brands/'+id+'/provider-markup',{method:'POST',headers:hdr(),body:JSON.stringify({providerMarkupPercent:Number($('madProviderMarkup').value||0)})});await api('/admin/brands/'+id+'/providers/assign',{method:'POST',headers:hdr(),body:JSON.stringify({providerCodes:selected.map(r=>r.dataset.providerRow)})});const old=new Map((detail?.providers||[]).map(x=>[String(x.providerCode||'').toUpperCase(),x]));for(const r of selected){const code=r.dataset.providerRow,o=old.get(code)||{},p=(detail?.platformProviders||[]).find(x=>String(x.code||'').toUpperCase()===code)||{};await api('/admin/brands/'+id+'/provider',{method:'POST',headers:hdr(),body:JSON.stringify({providerCode:code,ownership:'PLATFORM',enabled:Number(o.enabled??1),creditEnabled:Number(o.creditEnabled??1),lowCreditThreshold:Number(o.lowCreditThreshold??0),defaultChargePercent:Number(r.querySelector('[data-provider-override]')?.value||0),chargeBasis:String(o.chargeBasis||p.settlementCostBasis||'HOUSE_WIN')})});}}
