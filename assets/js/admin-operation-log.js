...
function renderPager(total){
  // The house ladder, from access-control-listing.js. This page counts pages from
  // 0, the ladder from 1.
  lastPage=Math.max(0,total-1);
  const host=$('pager');
  if(window.boAc && boAc.pageButtons){ host.innerHTML=boAc.pageButtons(page+1, Math.max(1,total)); return; }
  host.innerHTML='<button type="button" class="smart-page" data-page="'+page+'" aria-current="page">'+(page+1)+'</button>';
}
async function load(){const size=(window.boAc?boAc.resolve($('pageSize').value,document.querySelector('.table-card')):(Number($('pageSize').value)>0?Number($('pageSize').value):20));$('rows').innerHTML='<tr><td colspan="7" class="table-empty">Loading...</td></tr>';try{const d=await fetchLogicalPage(page,size),rows=d.rows||[];$('rows').innerHTML=rows.length?rows.map(x=>{let ok=true;try{ok=JSON.parse(x.afterJson||'{}').success!==false}catch(e){}return '<tr><td>'+esc((x.createdAt||'').replace('T',' '))+'</td><td><div class="table-primary">'+esc(x.actor||'SYSTEM')+'</div><small>#'+esc(x.entityId||'-')+'</small></td><td><b>'+esc((x.action||'-').replaceAll('_',' '))+'</b></td><td>'+esc(x.entityType||'-')+'</td><td>'+esc(x.ipAddress||'-')+'</td><td title="'+detail(x)+'">'+detail(x)+'</td><td><span class="standard-status '+(ok?'active':'inactive')+'"><i></i>'+(ok?'SUCCESS':'FAILED')+'</span></td></tr>'}).join(''):'<tr><td colspan="7" class="table-empty">No operation records found.</td></tr>';const from=rows.length?page*size+1:0,to=page*size+rows.length;$('showing').textContent='Showing '+from+' to '+to+' of '+d.total+' entries';const _c=$('opLogCount');if(_c)_c.textContent=d.total+' Record'+(Number(d.total)===1?'':'s');if(window.boAc&&String($('pageSize').value)===boAc.FIT&&!load._refit&&rows.length){const fit=boAc.resolve($('pageSize').value,document.querySelector('.table-card'));if(fit!==size){load._refit=true;setTimeout(()=>{load._refit=false;load()},0);return;}}renderPager(d.totalPages);}catch(e){$('rows').innerHTML='<tr><td colspan="7" class="table-empty">'+esc(e.message)+'</td></tr>';}}
$('pager').onclick=e=>{const b=e.target.closest('[data-page]');if(!b||b.disabled)return;page=Number(b.dataset.page);load()};$('pageSize').onchange=()=>{page=0;load()};/* No Refresh in this strip: the fields apply themselves (see the report contract). */$('action').onchange=()=>{page=0;load()};let _t=0;$('actor').oninput=()=>{clearTimeout(_t);_t=setTimeout(()=>{page=0;load()},400)};$('actor').onkeydown=e=>{if(e.key==='Enter'){clearTimeout(_t);page=0;load()}};load();
})();
