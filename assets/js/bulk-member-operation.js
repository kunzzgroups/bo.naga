(function(){
 ...
 function updatePromotionHelp(){const p=selectedPromotion(),help=$('bulkPromotionHelp');if(!help)return;if(!p){help.textContent='Select the promotion whose turnover / rollover / withdrawal policy should apply to this adjustment.';return}const wallet=String(p.freeCreditWallet||'MAIN_WALLET').replaceAll('_',' '),turnover=Number(p.turnover||0),rollover=Number(p.rollover||0),maxWithdraw=Number(p.maxWithdraw||0);help.textContent=`${p.name}${p.promotionCode?' ['+p.promotionCode+']':''} • Wallet: ${wallet} • Turnover: ${turnover}x • Rollover: ${rollover}x${maxWithdraw>0?' • Max withdraw: '+money(maxWithdraw):''}`}
