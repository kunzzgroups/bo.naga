(function(){
  const PAIRS=[['betFrom','betTo'],['txFrom','txTo'],['sessionFrom','sessionTo'],['ledgerFrom','ledgerTo'],['casinoFrom','casinoTo'],['reportFrom','reportTo'],['manualFrom','manualTo'],['wlFrom','wlTo'],['depositFrom','depositTo'],['withdrawFrom','withdrawTo'],['usageFrom','usageTo'],['dbgFrom','dbgTo']];
  ...
  function rangeFor(key){const now=new Date();let a=new Date(now),b=new Date(now);switch(key){case'today':break;case'yesterday':a.setDate(a.getDate()-1);b=new Date(a);break;case'this-week':a=startOfWeek(now);b=endOfWeek(now);break;case'last-week':a=startOfWeek(now);a.setDate(a.getDate()-7);b=endOfWeek(a);break;case'this-month':a=new Date(now.getFullYear(),now.getMonth(),1);b=new Date(now.getFullYear(),now.getMonth()+1,0);break;case'last-month':a=new Date(now.getFullYear(),now.getMonth()-1,1);b=new Date(now.getFullYear(),now.getMonth(),0);break;case'this-year':a=new Date(now.getFullYear(),0,1);b=new Date(now.getFullYear(),11,31);break;case'last-year':a=new Date(now.getFullYear()-1,0,1);b=new Date(now.getFullYear()-1,11,31);break;}return[a,b]}
